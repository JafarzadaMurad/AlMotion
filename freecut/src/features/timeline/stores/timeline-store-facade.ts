...
async function saveTimeline(projectId: string): Promise<void> {
  ...
  try {
    ...
    // Merge with local data for rootFolderHandle etc.
    const localData = await getProjectLocalData(projectId);
    if (localData) {
      Object.assign(project, {
        rootFolderHandle: localData.rootFolderHandle,
        rootFolderName: localData.rootFolderName,
        thumbnailId: localData.thumbnailId,
    ...
    // Update project timeline on backend
    await updateProjectApi(projectId, { timeline });

    // Save thumbnail reference to local data (client-only)
    if (thumbnailId) {
      await saveProjectLocalData(projectId, { thumbnailId });
    }
...
async function loadTimeline(
  ...
  try {
    const rawProject = await fetchProject(projectId);
    if (!rawProject) {
      throw new Error(`Project not found: ${projectId}`);
    }
    // Merge with local data
    const rawLocalData = await getProjectLocalData(projectId);
    if (rawLocalData?.schemaVersion) {
      rawProject.schemaVersion = rawLocalData.schemaVersion;
    }

    // New backend projects with no timeline don't need migration
    const hasTimeline = rawProject.timeline && rawProject.timeline.tracks && rawProject.timeline.tracks.length > 0;
    const storedSchemaVersion = rawProject.schemaVersion ?? (hasTimeline ? 1 : CURRENT_SCHEMA_VERSION);
    ...
    if (migrationResult.migrated || repairedLegacyLayouts.repaired) {
      ...
      // Persist migrated project timeline to backend
      await updateProjectApi(projectId, { timeline: project.timeline });
      // Save schema version to local data
      await saveProjectLocalData(projectId, { schemaVersion: CURRENT_SCHEMA_VERSION });
      logger.debug('Saved migrated project to backend');
    }
