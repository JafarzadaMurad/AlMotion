  ...
  return {
    ...
    async renderFrame(frame: number) {
      ...
      const {
        ...
        tracksByOrderDesc: tracksTopToBottom, // Render lowest order first (bottom) -> highest order last (on top)
        tracksByOrderAsc: sortedTracks,       // Occlusion cutoff checks from highest order down
        visibleTrackIds,
        activeTransitions,
