  ...
  try {
    ...
    if (!asrPipeline || currentModelId !== modelId) {
      ...
      try {
        ...
        logger.warn(
          `[FreeCut transcription] WebGPU initialization failed: ${gpuError instanceof Error ? gpuError.message : String(gpuError)
          }. Falling back to WASM (quantized).`
        );
        try {
          ...
          logger.warn(
            `[FreeCut transcription] Quantized WASM failed: ${wasmError instanceof Error ? wasmError.message : String(wasmError)
            }. Falling back to WASM (fp32).`
          );
          // Ultimate fallback: WASM with fp32 (no quantization)
          asrPipeline = await pipeline('automatic-speech-recognition', modelId, {
            device: 'wasm',
            dtype: 'fp32',
            progress_callback: progressCallback,
          });
          reportedEstimatedBytes = 0; // Reset as fp32 is much larger
          postMain({ type: 'runtime', info: { backend: 'wasm' } });
        }
      }
    ...
    postMain({
      type: 'error',
      message: `Failed to initialize Whisper model: ${error instanceof Error ? error.message : String(error)
        }`,
    });
  }
}
